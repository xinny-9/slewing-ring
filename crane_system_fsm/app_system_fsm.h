 ...
 * =============================================================================
 */
extern volatile uint8_t  g_fsm_update_flag;                 /* 50ms 状态机轮询执行标志 */
extern volatile uint8_t  g_single_step_only;                /* 单步调试模式标志：1-单步；0-连续 */
extern volatile uint32_t g_servo_reply_timeout_counter;     /* 舵机回读 10ms 软件超时计时器 */
extern volatile uint32_t g_sequence_settle_counter;         /* 搬运工步物理稳定时间计时器 */
extern volatile uint32_t g_settle_delay_counter;           /* 颗粒流体稳定时间计时器 */
